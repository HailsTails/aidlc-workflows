...
//
// t252 - the plugin sensor-manifest name guard. Sensor discovery
...
id: t252-good
kind: deterministic
command: bun {{HARNESS_DIR}}/tools/aidlc-sensor-t252-good.ts
default_severity: advisory
description: A well-named plugin sensor manifest for the t252 fixture
...
id: t252-bad
kind: deterministic
command: bun {{HARNESS_DIR}}/tools/aidlc-sensor-t252-bad.ts
default_severity: advisory
...
id: t252-nested
kind: deterministic
command: bun {{HARNESS_DIR}}/tools/aidlc-sensor-t252-nested.ts
default_severity: advisory
...
describe("t252 plugin sensor-manifest name guard", () => {
  let outcome: ComposeOutcome;

  beforeAll(() => {
    tmp = mkdtempSync(join(tmpdir(), "aidlc-t252-"));
    outcome = composePlugin({
      "sensors/aidlc-t252-good.md": GOOD_MANIFEST,
      "sensors/t252-bad.md": WRONG_PREFIX_MANIFEST,
      "sensors/subdir/aidlc-t252-nested.md": NESTED_MANIFEST,
    });
  }, TIMEOUT_MS);
  ...
  test("a wrong-prefix manifest drops-with-log naming the file and required shape", () => {
    ...
    expect(outcome.drops).toContain("t252-bad.md");
  ...
  test("a nested (but prefixed) manifest drops-with-log for the flat-scan miss", () => {
    expect(outcome.drops).toContain("subdir/aidlc-t252-nested.md");
    expect(outcome.drops).toContain("nested in a subdirectory");
  });

  test("the undiscoverable manifests never land in the install", () => {
    expect(existsSync(join(outcome.proj, ".claude", "sensors", "t252-bad.md"))).toBe(false);
    expect(
      existsSync(join(outcome.proj, ".claude", "sensors", "subdir", "aidlc-t252-nested.md")),
    ).toBe(false);
  });

  test("the well-named manifest lands and is discovered by the real sensor loader", () => {
    const landed = join(outcome.proj, ".claude", "sensors", "aidlc-t252-good.md");
    expect(existsSync(landed)).toBe(true);
    // {{HARNESS_DIR}} was substituted on copy (the composed manifest is real).
    expect(readFileSync(landed, "utf-8")).toContain("bun .claude/tools/aidlc-sensor-t252-good.ts");
    const ids = discoveredSensorIds(outcome.proj);
    expect(ids).toContain("t252-good");
    expect(ids).not.toContain("t252-bad");
    expect(ids).not.toContain("t252-nested");
  });
});
