...
// Ignored files stay excluded (git add semantics). A pathspec that no longer
// matches anything leaves the index empty - the empty-tree hash, which
// correctly reads as "changed" against any non-empty store fingerprint.
// Returns null when repoDir is not a git work tree or git is unavailable
// (callers report UNKNOWN, never a false STALE/CURRENT).
export function codekbScopeFingerprint(repoDir: string, paths: string[]): string | null {
  ...
  try {
    // Failures here (e.g. pathspec matched nothing) deliberately fall through
    // to write-tree: an empty index hashes to the empty tree, a real signal.
    spawnSync("git", ["add", "-A", "--", ...paths], { cwd: repoDir, env, encoding: "utf-8" });
