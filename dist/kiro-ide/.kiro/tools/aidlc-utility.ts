...
// not computable (non-git), which the block records verbatim.
//
...
function handleCodekbScopeDiff(projectDir: string, flags: Record<string, string>): void {
  ...
  if (flags.mint === "true") {
    ...
    const fp = codekbScopeFingerprint(repoDir, paths) ?? "unknown";
  ...
  const currentFingerprint =
    store.analyzedPaths.length > 0 ? codekbScopeFingerprint(repoDir, store.analyzedPaths) : null;
    ...
    emit(
      {
        ...
        detail: store.fingerprint === null ? "store has no fingerprint" : "fingerprint not computable here (not a git work tree)",
      },
      `UNVERIFIED: the store (intent: ${store.intent || "unrecorded"}) analyzed:\n${scopeLines}\n` +
        `but ${store.fingerprint === null ? "recorded no fingerprint" : "the current tree's fingerprint cannot be computed (not a git work tree)"} - freshness unknown.`,
    );
    return;
  }
