...
`bun .claude/tools/aidlc-orchestrate.ts report --stage reverse-engineering --result skipped --reason "codekb reuse: store CURRENT for <repo>, human chose reuse"`.
...
```
bun .claude/tools/aidlc-utility.ts codekb-scope-diff --repo <repo> --compare <record>/inception/reverse-engineering/scope-draft.md
```
