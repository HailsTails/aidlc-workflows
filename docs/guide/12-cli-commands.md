...
- **Compare mode** (`--compare <incoming timestamp.md>`) answers whether the
  ...
  exact paths and components an overwrite discards.
...
so it tracks working-tree content - rebases or squashes that rewrite history
do not fool it, and reverting an edit restores the original fingerprint.
